...
exports.run = async (client, message, args) => {
  ...
  if (!user)
    return message.reply("Kime altın yollayacağını etiketlemen lazım!");
  if (user.bot === true)
    return message.reply("altını bir bota yollarsan sen zararlı çıkarsın!");
  ...
  if (altın < mesaj) {
    ...
    const embed = new Discord.MessageEmbed()
      .setColor("RANDOM")
      .setDescription(
        `${user} Adlı kullanıcıya altın yollandı, yollanılan miktar: ${mesaj}`
      );
